#!/usr/bin/env python3
"""Ensure the ``InfraspecificRank`` pick list exists with MUSIT-aligned values.
...
Usage (from specify7-forms root):
  python scripts/ensure_infraspecific_rank_picklist.py
  python scripts/ensure_infraspecific_rank_picklist.py --apply
"""
...
def _load_values() -> list[str]:
    path = Path(__file__).resolve().parent.parent / "resources/picklists/infraspecific_rank.json"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Ensure InfraspecificRank pick list exists.")
    ...
    session, collection_name, collection_id = login(
        ...
        password,
        collection,
        log_prefix="ensure_infraspecific_rank_picklist",
    )

    picklist_name = "InfraspecificRank"
