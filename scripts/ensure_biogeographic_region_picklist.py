#!/usr/bin/env python3
"""Ensure the ``BiogeographicRegion`` pick list exists with MUSIT-aligned fylke values.
...
Usage (from specify7-forms root):
  python scripts/ensure_biogeographic_region_picklist.py
  python scripts/ensure_biogeographic_region_picklist.py --apply
"""
...
def _load_values() -> list[str]:
    path = Path(__file__).resolve().parent.parent / "resources/picklists/biogeographic_region.json"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Ensure BiogeographicRegion pick list exists.")
    ...
    session, collection_name, collection_id = login(
        ...
        password,
        collection,
        log_prefix="ensure_biogeographic_region_picklist",
    )

    picklist_name = "BiogeographicRegion"
