#!/usr/bin/env python3
"""Ensure the ``LocalityDatum`` pick list exists with migration-aligned values.
...
Usage (from specify7-forms root):
  python scripts/ensure_locality_datum_picklist.py
  python scripts/ensure_locality_datum_picklist.py --apply
"""
...
def _load_values() -> list[str]:
    path = Path(__file__).resolve().parent.parent / "resources/picklists/locality_datum.json"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Ensure LocalityDatum pick list exists.")
    ...
    session, collection_name, collection_id = login(
        ...
        password,
        collection,
        log_prefix="ensure_locality_datum_picklist",
    )

    picklist_name = "LocalityDatum"
